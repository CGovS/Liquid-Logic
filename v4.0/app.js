...
class LiquidLogicV4 {
  ...
  generateBoard() {
    ...
    let played = new Set();
    try { const s = localStorage.getItem('llPlayedV4'); if (s) played = new Set(JSON.parse(s)); } catch(e) {}
    ...
    try { localStorage.setItem('llPlayedV4', JSON.stringify([...played])); } catch(e) {}
  }
  ...
  renderGrid() {
    ...
    categories.forEach(cat => {
      ...
      cell.innerHTML = `<span class="cat-emoji">${cat.emoji}</span>${cat.title}`;
  ...
  handleBuzz(teamIdx) {
    ...
    if (this.state.gamePhase === 'READING') {
      if (!this.state.earlyBuzzPenalties.has(teamIdx)) {
        ...
        this.showNotification(`⚡ ${this.state.teams[teamIdx]}: WAIT FOR VOICE! Penalty applied.`);
      }
    } else if (this.state.gamePhase === 'OPEN') {
      if (this.state.penaltyActive && this.state.earlyBuzzPenalties.has(teamIdx)) return;
  ...
  activateBuzz(teamIdx) {
    ...
    this.state.timerInterval = setInterval(() => {
      ...
      bar.setAttribute('data-pct', String(this.state.timerValue * 25));
  ...
  onSpeechEnd() {
    ...
    if (this.state.earlyBuzzPenalties.size > 0) {
      ...
      setTimeout(() => { this.state.penaltyActive = false; }, 100);
    }
    this.renderBuzzerKeys();
  }
  ...
  startSipDistribution() {
    ...
    const sipText = pts === 1000 ? '5 sips 🔥' : sips === 1 ? '1 sip' : `${sips} sips`;
  ...
  handleNoWinner() { this.closeModalAndMarkUsed(); }
